...
comm_pract <- comm_pract_full_nocass %>% pull(yield)
...
comm_pract <- comm_pract_full_nocass %>% filter(climate == 0) %>% pull(yield)
irr_only   <- irr_only_full   %>% filter(climate == 0) %>% pull(yield)
nitro_only <- nitro_only_full %>% filter(climate == 0) %>% pull(yield)
all_recs   <- all_recs_full   %>% filter(climate == 0) %>% pull(yield)
...
irr_only   <- irr_only_full   %>% filter(climate == 1) %>% pull(yield)
nitro_only <- nitro_only_full %>% filter(climate == 1) %>% pull(yield)
all_recs   <- all_recs_full   %>% filter(climate == 1) %>% pull(yield)
...
comm_pract <-  comm_pract_full_nocass %>% filter(climate == 2) %>% pull(yield)
irr_only   <-  irr_only_full   %>% filter(climate == 2) %>% pull(yield)
nitro_only <-  nitro_only_full %>% filter(climate == 2) %>% pull(yield)
all_recs   <-  all_recs_full   %>% filter(climate == 2) %>% pull(yield)
...
comm_pract <-  comm_pract_full_nocass %>% pull(leaching)
irr_only   <-  irr_only_full   %>% pull(leaching)
nitro_only <-  nitro_only_full %>% pull(leaching)
...
comm_pract <- comm_pract_full_nocass %>% filter(climate == 0) %>% pull(leaching)
irr_only   <- irr_only_full   %>% filter(climate == 0)  %>% pull(leaching)
nitro_only <- nitro_only_full %>% filter(climate == 0)  %>% pull(leaching)
all_recs   <- all_recs_full   %>% filter(climate == 0)  %>% pull(leaching)
...
comm_pract <-  comm_pract_full_nocass %>% filter(climate == 1) %>% pull(leaching)
irr_only   <-  irr_only_full   %>% filter(climate == 1) %>% pull(leaching)
nitro_only <-  nitro_only_full %>% filter(climate == 1) %>% pull(leaching)
all_recs   <-  all_recs_full   %>% filter(climate == 1) %>% pull(leaching)
...
comm_pract <- comm_pract_full_nocass %>% filter(climate == 2) %>% pull(leaching)
irr_only   <- irr_only_full   %>% filter(climate == 2) %>% pull(leaching)
nitro_only <- nitro_only_full %>% filter(climate == 2) %>% pull(leaching)
all_recs   <- all_recs_full   %>% filter(climate == 2) %>% pull(leaching)
...
comm_pract <-  comm_pract_full_nocass %>% pull(wat_eff)
irr_only   <-  irr_only_full   %>% pull(wat_eff)
nitro_only <-  nitro_only_full %>% pull(wat_eff)
...
comm_pract <- comm_pract_full_nocass %>% filter(climate == 0) %>% pull(wat_eff)
irr_only   <- irr_only_full   %>% filter(climate == 0) %>% pull(wat_eff) 
nitro_only <- nitro_only_full %>% filter(climate == 0) %>% pull(wat_eff) 
all_recs   <- all_recs_full   %>% filter(climate == 0) %>% pull(wat_eff) 
...
comm_pract <-  comm_pract_full_nocass %>% filter(climate == 1) %>% pull(wat_eff)
irr_only   <-  irr_only_full   %>% filter(climate == 1) %>% pull(wat_eff)
nitro_only <-  nitro_only_full %>% filter(climate == 1) %>% pull(wat_eff)
all_recs   <-  all_recs_full   %>% filter(climate == 1) %>% pull(wat_eff)
...
comm_pract <- comm_pract_full_nocass %>% filter(climate == 2) %>% pull(wat_eff)
irr_only   <- irr_only_full   %>% filter(climate == 2) %>% pull(wat_eff)
nitro_only <- nitro_only_full %>% filter(climate == 2) %>% pull(wat_eff)
all_recs   <- all_recs_full   %>% filter(climate == 2) %>% pull(wat_eff)
